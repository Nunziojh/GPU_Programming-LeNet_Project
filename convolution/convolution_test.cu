...
for(r = 0; r < rounds; r++){
   /* //Prima versione, Base
    ...
    cudaMemcpy(host_output, dev_output, OUT_X * OUT_Y * OUT_Z * OUT_N * sizeof(float), cudaMemcpyDeviceToHost);
...
/*#ifdef DEBUG_PRINT
...
#ifdef DEBUG_PRINT
    debug_print(host_output, dev_output, "risultati_3D.txt", f_res, OUT_X, OUT_Y, OUT_Z, OUT_N);
#endif

    //Quinta versione, 3D con shared memory
#ifdef DEBUG_PRINT
    clean_vector<<<(unsigned int)((OUT_X * OUT_Y * OUT_Z * OUT_N) / min((OUT_X * OUT_Y * OUT_Z * OUT_N), 1024) + 1), min((OUT_X * OUT_Y * OUT_Z * OUT_N), 1024)>>>(dev_output, OUT_N * OUT_X * OUT_Y * OUT_Z);
#else
    init_values(host_input, host_kernel, dev_input, dev_kernel, dev_output);
    ...
    */
    ...
    // Da usare nella Backward dove le uscite sono dA3 e dA1
    // block = {(unsigned int)min(10, OUT_X), (unsigned int)min(10, OUT_Y), (unsigned int)(min(OUT_Z, 1024 / (min(10, OUT_X) * min(10, OUT_Y))))};
    // grid = {(unsigned int)ceil((float)OUT_X / block.x), (unsigned int)ceil((float)OUT_Y / block.y), (unsigned int)ceil((float)OUT_Z / block.z)};
    // block = {5, 5, 16};
    // grid = {2, 2, 2};
    // shared_mem_dim = (INPUT_X * INPUT_Y * INPUT_Z * ) * sizeof(float);
    // full_Convolution_shared<<<grid, block, shared_mem_dim>>>(dev_input, dev_kernel, dev_output, INPUT_X, INPUT_Y, INPUT_Z, KERNEL_X, KERNEL_Y, KERNEL_Z, KERNEL_N, OUT_X, OUT_Y, OUT_Z, PADDING);
...
#ifdef DEBUG_PRINT
    debug_print(host_output, dev_output, "risultati_3D_shared.txt", f_res, OUT_X, OUT_Y, OUT_Z, OUT_N);
#endif
}
