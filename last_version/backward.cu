...
#define PARAMETER_FILE "./epoch_5.txt"
...
int main(){
    ...
    */
    ...
    unsigned long int seed = 1711322078;
    srand(2);
    FILE *fp;
    ...
    */
    ...
    debug_print(kernels_first_layer_dev, 5, 5, 1, 6, "debug_file.txt");
    debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "debug_file.txt");
    debug_print(kernels_third_layer_dev, 5, 5, 16, 120, "debug_file.txt");
    debug_print(fc_first_layer_dev, 120, 84, 1, 1, "debug_file.txt");
    debug_print(fc_second_layer_dev, 84, 10, 1, 1, "debug_file.txt");
    ...
    */
    ...
    if((loss_file = fopen("loss_plot.txt", "a")) == NULL){
    ...
    if((prediction_file = fopen("predictions.txt", "a")) == NULL){
...
#define NUMBER_OF_STREAM 6
    ...
    */
    ...
    for(int epoch = 0; epoch < epoch_dim; epoch++){
        for(int batch = 0; batch < batch_dim; batch++){
            ...
            */
            ...
            block = {(unsigned int)in_w, (unsigned int)in_h}; //(unsigned int)min(32, max(in_w, out_w)), (unsinged int)min(32, max(in_h, out_h))
            grid = {(unsigned int)(in_w / 32 + 1), (unsigned int)(in_h / 32 + 1)}; //(unsigned int)max(in_w, out_w) / block.x + 1
            shared_mem_dim = (in_h * in_w + KERNEL_DIM * KERNEL_DIM) * sizeof(float);
            for(int i = 0; i < kernel_num_first_layer; i++){
                convolution<<<grid, block, shared_mem_dim/*, stream[i%NUMBER_OF_STREAM]*/>>>(img_dev, first_conv + (i * out_h * out_w), kernels_first_layer_dev + (i * KERNEL_DIM * KERNEL_DIM), in_h, out_h, KERNEL_DIM, padding, stride_c);
            }
            ...
            // debug_print(first_conv, out_w, out_h, 6, 1, "opt_first_conv.txt");
            ...
            */
            ...
            // debug_print(first_pool, out_w, out_h, 6, 1, "opt_first_pool.txt");
            ...
            */
            ...
            out_h = (in_h + 2 * padding - KERNEL_DIM) / stride_c + 1;
            out_w = (in_w + 2 * padding - KERNEL_DIM) / stride_c + 1;
            block = {(unsigned int)in_w, (unsigned int)in_h};
            grid = {(unsigned int)(in_w / 32 + 1), (unsigned int)(in_h / 32 + 1)};
            shared_mem_dim = (in_h * in_w + KERNEL_DIM * KERNEL_DIM) * sizeof(float);
            ...
            for(int i = 0; i < kernel_num_second_layer; i++){
                for(int j = 0; j < kernel_num_first_layer; j++){
                    convolution3D<<<grid, block, shared_mem_dim/*, stream[i%NUMBER_OF_STREAM]*/>>>(first_pool + (j * in_h * in_w), second_conv + (i * out_h * out_w), kernels_second_layer_dev + (j * KERNEL_DIM * KERNEL_DIM + (i * KERNEL_DIM * KERNEL_DIM * kernel_num_first_layer)), in_h, out_h,KERNEL_DIM, padding, stride_c);
                }
            }
            cudaDeviceSynchronize();//Potrebbe servire solo se ci sono gli Stream
            ...
            // debug_print(second_conv, out_w, out_h, 16, 1,"opt_second_conv.txt");
            ...
            */
            ...
            cudaDeviceSynchronize();
            // debug_print(second_pool, out_w, out_h, 16, 1, "opt_second_pool.txt");
            ...
            */
            ...
            for(int i = 0; i < kernel_num_third_layer; i++){
                for(int j = 0; j < kernel_num_second_layer; j++){
                    convolution3D<<<grid, block, shared_mem_dim, stream[i%NUMBER_OF_STREAM]>>>(second_pool + (j * in_h * in_w), third_conv + (i * out_h * out_w), kernels_third_layer_dev + (j * KERNEL_DIM * KERNEL_DIM + (i * KERNEL_DIM * KERNEL_DIM * kernel_num_second_layer)), in_h, out_h, KERNEL_DIM, padding, stride_c);
                }
            }
            cudaDeviceSynchronize();
            ...
            // debug_print(third_conv, out_w, out_h, 120, 1, "opt_third_conv.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_1.txt");
            ...
            */
            ...
            out_h = fc_second_dim;
            out_w = m;
            block = {(unsigned int)TILE_DIM, (unsigned int)TILE_DIM}; // (unsigned int)min(32, max(in_w,TILE_DIM)), (unsigned int)min(32, (in_h,TILE_DIM))
            grid = {(unsigned int)(in_w / block.x + 1), (unsigned int)(h_2 / block.y + 1)};
            shared_mem_dim = (TILE_DIM * TILE_DIM * 2) * sizeof(float);
            matrix_product<<<grid, block, shared_mem_dim>>>(fc_first_layer_dev, third_conv, second_fc, m, fc_second_dim, fc_first_dim, TILE_DIM);
            tanh<<<grid, block>>>(second_fc, 1, fc_second_dim);

            // debug_print(second_fc, 1, 84, 1, 1, "opt_second_fc.txt");
            ...
            */
            ...
            grid = {(unsigned int)(in_w / block.x + 1), (unsigned int)(h_2 / block.y + 1)};//(unsinged int)max(in_w, out_w) / block.x + 1
            shared_mem_dim = (TILE_DIM * TILE_DIM * 2) * sizeof(float);
            matrix_product<<<grid, block, shared_mem_dim>>>(fc_second_layer_dev, second_fc, third_fc, m, fc_third_dim, fc_second_dim, TILE_DIM);
            // debug_print(third_fc, 1, 10, 1, 1, "opt_third_fc.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_2.txt");
            ...
            */
            ...
            // printf("Prediction Index right after the for loop: %d\n", prediction_index);
            ...
            */
            ...
            // debug_print(dZ2, 1, 10, 1, 1, "opt_dZ2.txt");
            ...
            */
            ...
            out_w = fc_second_dim;
            block = {(unsigned int)TILE_DIM, (unsigned int)TILE_DIM};
            grid = {(unsigned int)(h_2/ block.x + 1), (unsigned int)(in_h / block.y + 1)};
            shared_mem_dim = (TILE_DIM * TILE_DIM * 2) * sizeof(float);
            matrix_product_transpose<<<grid, block, shared_mem_dim>>>(dZ2, second_fc, dW2, out_w, out_h, in_w, TILE_DIM);
            // debug_print(dW2, 84, 10, 1, 1, "opt_bef_dW2.txt");
            matrix_scalar_product<<<grid, block>>>(dW2, (1.0 / m), out_w, out_h);
            // debug_print(dW2, 84, 10, 1, 1, "opt_dW2.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_3.txt");
            ...
            */
            ...
            out_w = m;
            block = {(unsigned int)TILE_DIM, (unsigned int)TILE_DIM};
            grid = {(unsigned int)(in_h / block.x + 1), (unsigned int)(out_h / block.y + 1)};
            shared_mem_dim = (TILE_DIM * TILE_DIM * 2) * sizeof(float);
            matrix_transpose_product<<<grid, block, shared_mem_dim>>>(fc_second_layer_dev, dZ2, dZ1, out_w, out_h, in_h, TILE_DIM);
            ...
            // debug_print(dZ1, 1, 84, 1, 1, "opt_dZ1.txt");
            ...
            */
            ...
            out_w = fc_first_dim;
            block = {(unsigned int)TILE_DIM, (unsigned int)TILE_DIM};
            grid = {(unsigned int)(out_w/ block.x + 1), (unsigned int)(out_h / block.y + 1)};
            shared_mem_dim = (TILE_DIM * TILE_DIM * 2) * sizeof(float);
            matrix_product_transpose<<<grid, block, shared_mem_dim>>>(dZ1, third_conv, dW1, out_w, out_h, in_w, TILE_DIM);
            matrix_scalar_product<<<grid, block>>>(dW1, (1.0 / m), out_w, out_h);

            // debug_print(dW1, 120, 84, 1, 1, "opt_dW1.txt");
            ...
            */
            ...
            grid = {(unsigned int)(in_h/ block.x + 1), (unsigned int)(out_h / block.y + 1)};
            shared_mem_dim = (TILE_DIM * TILE_DIM * 2) * sizeof(float);
            matrix_transpose_product<<<grid, block, shared_mem_dim>>>(fc_first_layer_dev, dZ1, dZ0, out_w, out_h, in_h, TILE_DIM);
            ...
            // debug_print(dZ0, 1, 120, 1, 1, "opt_dZ0.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_4.txt");
            ...
            */
            ...
            block = {(unsigned int)out_w, (unsigned int)out_h};
            grid = {(unsigned int)(out_w / 32 + 1), (unsigned int)(out_h / 32 + 1)};
            shared_mem_dim = (in_h * in_w + h_2 * w_2) * sizeof(float);
            ...
            for(int i = 0; i < kernel_num_third_layer; i++){
                for(int j = 0; j < kernel_num_second_layer; j++){
                    convolution3D<<<grid, block, shared_mem_dim/*, stream[j%NUMBER_OF_STREAM]*/>>>(second_pool + (j * in_h * in_w), dF2 + (j * out_w * out_h + (i * out_w * out_h * kernel_num_second_layer)), dZ0 + (i * h_2 * w_2), in_h, out_h, h_2, padding, stride_c);
                }
            }
            // cudaDeviceSynchronize();

            // debug_print(dF2 + (5 * 5 * 16 * 119), 5, 5, 16, 1, "opt_dF2.txt");
            ...
            */
            ...
            for(int i = 0; i < kernel_num_third_layer; i++){
                for(int j = 0; j < kernel_num_second_layer; j++){
                    convolution3D<<<grid, block, shared_mem_dim/*, stream[j%NUMBER_OF_STREAM]*/>>>(kernels_third_layer_dev + (j * in_h * in_w + (i * in_h * in_w * kernel_num_second_layer)), dA3 + (j * out_h * out_w), dZ0 + (i * h_2 * w_2), in_h, out_h, h_2, padding_full_conv, stride_c);
                }
            }
            // cudaDeviceSynchronize();

            // debug_print(dA3, 5, 5, 16, 1, "opt_dA3.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_5.txt");
            ...
            */
            ...
            // debug_print(dP1, 5, 5, 16, 1, "opt_dP1.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_6.txt");
            ...
            */
            ...
            // debug_print(dA2, 10, 10, 16, 1, "opt_dA2.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_7.txt");
            ...
            */
            ...
            // debug_print(dC1, 10, 10, 16, 1, "opt_dC1.txt");
            // debug_print(kernels_second_layer_dev, 5, 5, 6, 16, "opt_8.txt");
            ...
            */
            ...
            out_h = (in_h + 2 * padding_full_conv - h_2) / stride_c + 1;
            out_w = (in_w + 2 * padding_full_conv - w_2) / stride_c + 1;
            block = {(unsigned int)(in_w + 2 * padding_full_conv), (unsigned int)(in_h + 2 * padding_full_conv)}; //max(out_w, in_dim + 2 * padding_full_conv)
            grid = {(unsigned int)(block.x / 32 + 1), (unsigned int)(block.y / 32 + 1)};
            shared_mem_dim = ((in_h + 2 * padding_full_conv) * (in_w + 2 * padding_full_conv) + h_2 * w_2) * sizeof(float);
            clean_vector<<<2, 1024>>>(dA1, 14 * 14 * 6);// 14 * 14 * 6 = 1176, servono due blocchi, provare ad usarne due da 600 invece che da 1024.
            for(int i = 0; i < kernel_num_second_layer; i++){
                for(int j = 0; j < kernel_num_first_layer; j++){
                    convolution3D<<<grid, block, shared_mem_dim/*, stream[j%NUMBER_OF_STREAM] */>>>(kernels_second_layer_dev + (j * in_h * in_w + (i * in_h * in_w * kernel_num_first_layer)), dA1 + (j * out_h * out_w), dC1 + (i * h_2 * w_2), in_h, out_h, h_2, padding_full_conv, stride_c);
                    //cudaDeviceSynchronize();
                }
            }
            // cudaDeviceSynchronize();
            // debug_print(dA1, 14, 14, 6, 1, "opt_dA1.txt");
            ...
            */
            ...
            out_h = KERNEL_DIM;
            out_w = KERNEL_DIM;
            block = {(unsigned int)(in_w + 2 * padding_full_conv), (unsigned int)(in_h + 2 * padding_full_conv)};
            grid = {(unsigned int)(block.x / 32 + 1), (unsigned int)(block.y / 32 + 1)};
            shared_mem_dim = ((in_h + 2 * padding_full_conv) * (in_w + 2 * padding_full_conv) + h_2 * w_2) * sizeof(float);
            clean_vector<<<3, 1024>>>(dF1, 5 * 5 * 6 * 16);// 5 * 5 * 6 * 16 = 2400, servono tre blocchi, provare ad usarne tre da 800 invece che da 1024.
            for(int i = 0; i < kernel_num_second_layer; i++){
                for(int j = 0; j < kernel_num_first_layer; j++){
                    convolution3D<<<grid, block, shared_mem_dim/*, stream[j%NUMBER_OF_STREAM] */>>>(first_pool + (j * in_h * in_w), dF1 + (j * out_h * out_w + (i * out_h * out_w * kernel_num_first_layer)), dC1 + (i * h_2 * w_2), in_h, out_h, h_2, padding, stride_c);
                    //cudaDeviceSynchronize();
                }
            }
            // cudaDeviceSynchronize();

            // debug_print(dF1, 5, 5, 6, 1, "opt_dF1.txt");
            ...
            */
            ...
            // debug_print(dP0, 14, 14, 6, 1, "opt_dP0.txt");
            ...
            */
            ...
            // cudaDeviceSynchronize();
            // debug_print(dA0, 14, 14, 6, 1, "opt_dP0.txt");
            ...
            */
            ...
            // debug_print(dC0, 28, 28, 6, 1, "opt_dC0.txt");
            ...
            */
            ...
            for(int i = 0; i < kernel_num_first_layer; i++){
                convolution3D<<<grid, block, shared_mem_dim/*, stream[i%NUMBER_OF_STREAM] */>>>(img_dev, dF0 + (i * out_h * out_w), dC0 + (i * h_2 * w_2), in_h, out_h, h_2, padding, stride_c);
            }

            debug_print(dF0, 5, 5, 1, 6, "opt_dF0.txt");
...
#ifdef TEST
#ifndef USAGE
    FILE *precision_file = fopen("precision.txt", "w");
    fprintf(precision_file, "Valori predetti correttamente, su %d: %d\n", batch_dim, prediction_counter);
    fclose(precision_file);
#else
    ...
    for(int i = 0; i < NUMBER_OF_STREAM; i++) cudaStreamDestroy(stream[i]);
